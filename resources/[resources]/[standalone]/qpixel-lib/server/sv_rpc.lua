Resource, Promises, Functions, CallIdentifier = GetCurrentResourceName(), {}, {}, 0
...
function RPC.execute(name, ...)
    ...
    TriggerClientEvent("rpc:request", Resource, name, callID, ParamPacker(...), true)

    Citizen.SetTimeout(20000, function()
        if not solved then
            Promises[callID]:resolve({nil})
            TriggerClientEvent("rpc:server:timeout", Resource, name)
        end
    end)
