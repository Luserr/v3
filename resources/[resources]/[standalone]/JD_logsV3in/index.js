...
for (const x in config.tokens) {
    ...
    if(config.tokens[x].token !== ''){
        ...
        client[x].on("ready", async () => {
            let dest = false
            client[x].user.setPresence({ activities: [{ name: `Venture RP Logging: ${x}/${Object.keys(config.tokens).length}`, type: "PLAYING" }], afk: true, status: 'dnd' });
            if(x == '1'){
                try{
...
if(GetCurrentResourceName() !== GetInvokingResource()) return 505
    ...
    const embed = new MessageEmbed()
        ...
        .setTimestamp()
        .setFooter({text: `Venture RP Logs`})
