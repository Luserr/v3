    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        ...
        if ( GetVehicleClass( vehicle ) == 18 ) then
            ...
            if ( radarEnabled ) then 
                QBCore.Functions.Notify("Radar enabled.", "success")
            else 
                QBCore.Functions.Notify("Radar disabled.", "error")
            end 
...
function ToggleSpeedType()
    if ( radarInfo.speedType == "mph" ) then 
        ...
        QBCore.Functions.Notify("Speed type set to Km/h.")
    else 
        radarInfo.speedType = "mph"
        QBCore.Functions.Notify("Speed type set to MPH.")
    end
end 
...
function ToggleLockBeep()
    if ( radarInfo.lockBeep ) then 
        ...
        QBCore.Functions.Notify("Radar fast lock beep disabled.", "error")
    else 
        radarInfo.lockBeep = true
        QBCore.Functions.Notify("Radar fast lock beep enabled.", "success")
    end    
end 
