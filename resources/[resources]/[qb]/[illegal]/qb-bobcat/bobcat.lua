...
function ThermiteEffect()
   RequestAnimDict("anim@heists@ornate_bank@thermal_charge")
    ...
    --TriggerServerEvent('qb-doorlock:server:updateState', 'bobcath-bobcatfrontdoor', false, false, false, true, false, false)
end
            ...
            if cops >= Config.Cops then
                QBCore.Functions.TriggerCallback("gc-bobcatheist:Cooldown", function(cooldown)
                    if not Cooldown then
                        if cops >= 0 then
                            if exports['qb-inventory']:HasItem('thermite') then
                                Alert()
...
function ThermiteEffect2()
    ...
    --TriggerServerEvent('qb-doorlock:server:updateState', 'bobcath-bobcatsecdoor', false, false, false, true, false, false)
end
...
local function hackanim()
    ...
    local animPos3 = GetAnimInitialOffsetPosition(animDict, "hack_exit", loc.x + 0, loc.y + 0, loc.z + 1.15)
        if exports['qb-inventory']:HasItem('greenlaptop') then
            ...
            TriggerServerEvent("qb-smallresources:server:RemoveItem", "greenlaptop", 1)
            --QBCore.Functions.RemoveItem('greenlaptop')
            Wait(6000)
...
RegisterNetEvent('gc-bobcatheist:client:Lockdown', function()
    ...
    --TriggerServerEvent('qb-doorlock:server:updateState', 'bobcath-bobcatfrontdoor', true, false, false, true, false, false)
    --TriggerServerEvent('qb-doorlock:server:updateState', 'bobcath-bobcatsecdoor', true, false, false, true, false, false)
    --TriggerServerEvent('qb-doorlock:server:updateState', 'bobcath-bobcatsecdoor', true, false, false, true, false, false)
...
function BombPlanting()
    ...
    TriggerServerEvent("qb-smallresources:server:RemoveItem", "weapon_pipebomb", 1)
    --QBCore.Functions.RemoveItem('weapon_pipebomb')
