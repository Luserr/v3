                            ...
                            if exports['qb-inventory']:HasItem("thermite") then
                                ...
                                function() -- Successfully Disable Cameras
                                    ...
                                    if Config.DoorLock == "np" then
                                        ...
                                        TriggerServerEvent('qb-doorlock:server:updateState', Config.QBDoorID, false, false, false, true, false, false)
                                    else
...
RegisterNetEvent('hackinglaptop:UseHackinglaptop', function()
    local pos = GetEntityCoords(PlayerPedId())
    if #(pos - vector3(Config.JewelLocation["DisableCameras"].x, Config.JewelLocation["DisableCameras"].y,Config.JewelLocation["DisableCameras"].z)) < 30.0 then
            if Config.JewelLocation["ThermiteSecurity"].isDone then
                if exports['qb-inventory']:HasItem("hacking-laptop") then
                    exports['qb-dispatch']:VangelicoRobbery(camId)
                    TriggerEvent('inventory:client:requiredItems', requiredItems, false)
...
function ThermiteSuccess()
    ...
    local pos = GetEntityCoords(PlayerPedId())
    if #(pos - vector3(Config.JewelLocation["ThermiteSecurity"].x, Config.JewelLocation["ThermiteSecurity"].y,Config.JewelLocation["ThermiteSecurity"].z)) < 30.5 then
