...
RegisterServerEvent("nc-forgery:server:ForgeID")
AddEventHandler("nc-forgery:server:ForgeID", function(citizenid, firstname, lastname, birthday, nationality)
    local src = source
...
RegisterServerEvent("nc-forgery:server:ForgeDrivers")
AddEventHandler("nc-forgery:server:ForgeDrivers", function(firstname,lastname,birthday)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.money.cash >= Config.Cost then
        info = {}
