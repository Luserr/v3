...
local function ProcessHeroin()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('qb-drugprocessing:processPoppyResin')
...
RegisterNetEvent('qb-drugprocessing:ProcessPoppy', function()
...
RegisterNetEvent("qb-drugprocessing:processHeroin",function()
	QBCore.Functions.TriggerCallback('qb-drugprocessing:validate_items', function(result)
		if result then
			ProcessHeroin()
		else
...
RegisterNetEvent("qb-drugprocessing:pickHeroin", function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		...
		QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.collecting"), 10000, false, true, {
			...
			TriggerServerEvent('qb-drugprocessing:pickedUpPoppy')
			isPickingUp = false
