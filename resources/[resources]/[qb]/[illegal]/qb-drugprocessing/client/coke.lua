...
local function ProcessCoke()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('qb-drugprocessing:processCocaLeaf')
...
local function CutCokePowder()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('qb-drugprocessing:processCocaPowder')
...
local function ProcessBricks()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.packing"), 15000, false, true, {
		...
		TriggerServerEvent('qb-drugprocessing:processCocaBrick')
...
RegisterNetEvent('qb-drugprocessing:ProcessCocaFarm', function()
...
RegisterNetEvent('qb-drugprocessing:ProcessCocaPowder', function()
	local coords = GetEntityCoords(PlayerPedId())
	local amount = 10
	local amount2 = 5
...
RegisterNetEvent('qb-drugprocessing:ProcessBricks', function()
...
RegisterNetEvent('qb-drugprocessing:EnterCWarehouse', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('qb-drugprocessing:ExitCWarehouse', function()
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
...
RegisterNetEvent('qb-drugprocessing:pickCocaLeaves', function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		if not isPickingUp then
			...
			QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.collecting"), 10000, false, true, {
				...
				TriggerServerEvent('qb-drugprocessing:pickedUpCocaLeaf')
				isPickingUp = false
			end, function()
				ClearPedTasks(playerPed)
				isPickingUp = false
