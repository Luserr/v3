$(function() {
    window.addEventListener('message', function(event) {
        if (event.data.type === "OpenReception"){
            ...
            TumunuKapat();
            $.post('http://qb-voncrastenburg/CloseReception', JSON.stringify({}));
        }
    });
});
...
$(document).on('click','#KapatRecepsiyon2',function(e){
    ...
    $.post('http://qb-voncrastenburg/CloseReception2', JSON.stringify({}));
})
...
$(document).on('click','#LeftOrospu',function(e){
    ...
    $.post('http://qb-voncrastenburg/LeftMotelRoom', JSON.stringify({
...
$(document).on('click','#ReKey',function(e){
    ...
    $.post('http://qb-voncrastenburg/CopyMotelKeyRoom', JSON.stringify({
        RoomNo: motelid,
    })); 
})
...
$(document).on('click','#KiralaOrospu',function(e){
    ...
    motelid = $(this).attr("data-motelid")
    $.post('http://qb-voncrastenburg/BuyMotelRoom', JSON.stringify({
        RoomNo: odaid,
        OdaFiyat: odaprice,
        MotelId: motelid
    })); 
