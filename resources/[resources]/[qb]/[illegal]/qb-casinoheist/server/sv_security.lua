...
RegisterNetEvent('qb-casinoheist:server:ManagementComputer', function()
    ...
    local item = exports['qb-inventory']:GetItemByName(src, 'casino_usb1')
    if item then
        ...
        exports['qb-inventory']:AddItem(src, 'casino_accesscode1', 1, false, info)
...
RegisterNetEvent('qb-casinoheist:server:SecretaryComputer', function()
    ...
    local item = exports['qb-inventory']:GetItemByName(src, 'casino_usb2')
    if item then
        exports['qb-inventory']:RemoveItem(src, item.name, 1, item.slot)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[item.name], 'remove', 1)
        
        local info = {code = string.sub(keycardCode, #keycardCode/2+1, #keycardCode)}
        exports['qb-inventory']:AddItem(src, 'casino_accesscode2', 1, false, info)
...
RegisterNetEvent('qb-casinoheist:server:GrabKeyCard', function()
    ...
    exports['qb-inventory']:AddItem(src, 'casino_keycard', 1, false)
