    ...
    while true do
        ...
        if dist <= 1.5 then
            ...
            if IsControlJustReleased(0, 38) then
                ...
                TriggerServerEvent("nc-grandma:server:hug", spawn)
                Citizen.Wait(5000)
                FreezeEntityPosition(ped, false)
                TriggerServerEvent("nc-grandma:server:idle", spawn)
                TriggerServerEvent('nc-grandma:server:SetResourceCooldown')
            end
        else
            Citizen.Wait(5000)
        end
        if inZone and not alreadyEnteredZone then
            ...
            if Config.Framework.DrawText == 'nchub' then
                exports['nchub']:DrawText(text, 'top')
            ...
            options = {
                {
                    num = 1,
                    type = "client",
                    event = "nc-grandma:client:MedicalAidInjuries",
...
RegisterNetEvent('nc-grandma:client:hug', function(coords)
    local model = GetClosestHug(coords)
    TaskPlayAnim(model, "mp_ped_interaction", "kisses_guy_a", 8.00, -8.00, 5000, 51, 0.00, 0, 0, 0)
end)

RegisterNetEvent('nc-grandma:client:idle', function(coords)
    local model = GetClosestHug(coords)
    TaskPlayAnim(model, "amb@world_human_aa_smoke@male@idle_a", "idle_c", 8.00, -8.00, -1, 51, 0.00, 0, 0, 0)
end)

RegisterNetEvent("nc-grandma:client:MedicalAid", function()
        ...
        QBCore.Functions.Progressbar('ConstantDevelopmentIllegalGrandmaMedical', Config.IllegalMedical.PedName..' is helping you..', math.random(2500, 10000), false, true, {
            ...
            if Config.IllegalMedical.Minigame.Enabled == true then
                if Config.IllegalMedical.Minigame.PSUI then
                    exports['nc-ui']:Circle(function(success)
                        if success then
                            TriggerServerEvent("nc-grandma:server:MedicalAid")
                        else
...
RegisterNetEvent("nc-grandma:client:MedicalAidInjuries", function()
    QBCore.Functions.Progressbar('ConstantDevelopmentIllegalGrandmaMedical', Config.IllegalMedical.PedName..' is helping you..', math.random(2500, 10000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        ...
        if Config.IllegalMedical.Minigame.Enabled == true then
            if Config.IllegalMedical.Minigame.PSUI then
                exports['nc-ui']:Circle(function(success)
                    if success then
                        TriggerServerEvent("nc-grandma:server:MedicalAidInjuries")
                    else
                        ConstantDevelopmentGrandma(3, "You failed my Challenge and expect me to help you? Are you insane...", Config.IllegalMedical.OkOkNotificationTitle)
                    end
