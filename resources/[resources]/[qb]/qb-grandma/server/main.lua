...
RegisterNetEvent('nc-grandma:server:hug', function(coords)
    TriggerClientEvent("nc-grandma:client:hug", -1, coords)
end)

RegisterNetEvent('nc-grandma:server:idle', function(coords)
    TriggerClientEvent("nc-grandma:client:idle", -1, coords)
end)

RegisterNetEvent('nc-grandma:server:MedicalAid', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nc-grandma:server:MedicalAidInjuries', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local HasCash = Player.Functions.GetMoney("cash")
    local HasBank = Player.Functions.GetMoney("bank")
