RegisterNUICallback("data_status", function(data)
    if soundInfo[data.id] ~= nil then
        if data.type == "finished" then
            ...
            TriggerEvent("np-musicplayer:songStopPlaying", data.id)
        end
...
RegisterNetEvent("np-musicplayer:stateSound")
AddEventHandler("np-musicplayer:stateSound", function(state, data)
