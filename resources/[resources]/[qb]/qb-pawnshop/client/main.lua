...
RegisterNetEvent('venture-goldbarsmelt2', function()
	---
end)

RegisterNetEvent('venture-goldbarsmelt', function(data)
	QBCore.Functions.TriggerCallback('qb-pawnshop:server:getInv', function(inventory)
		local barmenu = {
			{
				header = "Sell Gold Bars for full price",
				isMenuHeader = true,
			}
		}
		for k,v in pairs(inventory) do
			for i=1, #data.items do
				if v.name == data.items[i].item then
					barmenu[#barmenu +1] = {
						header = QBCore.Shared.Items[v.name].label,
						txt = "Sell Gold Bars for "..data.items[i].price.." now",
						params = {
							event = "venture-goldbarsmelt2",
							args = {
								label = QBCore.Shared.Items[v.name].label,
								price = data.items[i].price,
								name = v.name,
		...
		barmenu[#barmenu+1] = {
			header = "Back",
			params = {
				event = 'venture-bankrobbery:client:AllKnowingGuy'
			}
		}
...
RegisterNetEvent('venture-bankrobbery:client:AllKnowingGuy', function()
	local goldbars = {
		{
			header = "All Knowing Guy",
			isMenuHeader = true,
		},
		{
			header = "Gold Bars?",
			txt = "Smelt Gold Bars",
			params = {
				event = "venture-goldbarsmelt",
				args = {
					items = Config.GoldBarSmelt
				}
			}
		}
	}
	exports['qb-menu']:openMenu(goldbars)
end)

--[[exports['qb-target']:AddBoxZone('venturegoldbars', vector3(1076.2008, -1978.976, 31.47126), 1, 1, {
	name='venturegoldbars',
	heading=57.45,
	debugPoly=false,
	minZ = 30.04,
	maxZ = 32.64,
