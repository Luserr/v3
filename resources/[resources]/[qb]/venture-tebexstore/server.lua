local DiscordWebhook = 'https://discord.com/api/webhooks/1016069955306918020/rnfkt_mEbtgAI5PP-ra-sYmTQxCvxirRKKGxV8XA_VpuRWyY35McW0T04jRrLavWav8a'
local redeemedCars = {}
...
local DISCORD_NAME = "VentureRP Reloaded Tebex Store"
		...
		local connect = {
			{
				...
				["description"] = message,
				["footer"] = {
					["text"] = "VentureRP Reloaded Tebex Store",
				},
			}
		}
							...
							if v.PackageName == i then
								for j=1, #v.Items, 1 do
									...
									if counter.type == 'item' or counter.type == 'weapon' then
										...
										QBCore.Functions.AddPermission(player.PlayerData.source, counter.rank)
									elseif counter.type == 'account' then
										local src = source
