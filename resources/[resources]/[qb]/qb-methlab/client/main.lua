...
function CreateMethVehicle()
    ...
    local plt = GetVehicleNumberPlateText(methVehicle)
		...
		exports['qb-fuel']:SetFuel(methVehicle, 100)
...
function PoliceCall(vehicle)
    ...
    TriggerServerEvent(
        ...
        "10-34",
        "Locals report possible drug sales",
...
CreateThread(function()
    ...
    while true do
		...
		if dropOff6 < 1.6 and not MethRun then
			...
			DrawText3Ds(MethWorker["x"],MethWorker["y"],MethWorker["z"], "[E] Pickup Chemicals (Cash Payment)") 
			if IsControlJustReleased(0,38) then
