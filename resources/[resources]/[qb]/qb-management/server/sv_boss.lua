...
function AddMoney(account, amount)
	...
	exports['Renewed-Banking']:addAccountMoney(account, amount, "Management")
...
function RemoveMoney(account, amount)
	...
	if amount > 0 then
		exports['Renewed-Banking']:removeAccountMoney(account, amount, "Management")
	end
	return isRemoved
end
...
RegisterNetEvent("qb-bossmenu:server:withdrawMoney", function(amount)
	...
	if RemoveMoney(job, amount) then
		...
		exports['Renewed-Banking']:handleTransaction(src, "business", -amount, "deposit", joblabel, (note ~= "" and note or "Withdraw from ".. joblabel .."'s account from Business Computer."))
	else
...
RegisterNetEvent("qb-bossmenu:server:depositMoney", function(amount)
	...
	if Player.Functions.RemoveMoney("cash", amount) then
		...
		exports['Renewed-Banking']:handleTransaction(src, "business", amount, "deposit", joblabel, note ~= "" and note or "Deposited cash into ".. joblabel .."'s business account from Business Computer.")
	else
...
QBCore.Functions.CreateCallback('qb-bossmenu:server:GetAccount', function(_, cb, jobname)
	local result = exports['Renewed-Banking']:getAccountMoney(jobname)
...
RegisterNetEvent('qb-bossmenu:server:FireEmployee', function(target)
	...
	local EmployeeCitizenId = Employee.PlayerData.citizenid
    if not Player.PlayerData.job.isboss then ExploitBan(src, 'FireEmployee Exploiting') return end
    local data = {
    ...
    if Employee then
        if target ~= Player.PlayerData.citizenid then
            if Employee.Functions.SetJob("unemployed", '0') then
				exports['qb-phone']:fireUser(EmployeeCurrentJob, EmployeeCitizenId)
