...
AddEventHandler('nc_delivery:Delivey', function(info)
  local src = source
      ...
      for k,v in pairs(Orders) do Citizen.Wait(0)
        if now > v.cancelTime  then
          if v.source ~= nil then 
            ...
            TriggerClientEvent("nc_delivery:CancelOrder",v.source)
          end
          Orders[k] = nil 
        end
      end
...
RegisterNetEvent('nc_delivery:SendMeOrders')
AddEventHandler('nc_delivery:SendMeOrders', function()
  local src = source
  TriggerClientEvent("nc_delivery:OpenOrders",src,Orders,os.time())
  PlayersUsedUi[src] = true
end)


RegisterNetEvent('nc_delivery:NewOrderNotify')
AddEventHandler('nc_delivery:NewOrderNotify', function(value)
  local src = source
  if value then
...
RegisterNetEvent('nc_delivery:CanIOrder')
AddEventHandler('nc_delivery:CanIOrder', function(id)
  local src = source
  ...
  if OnAnyOrder(src) then  singleNotify(src,"error",U["you_have_already"],5000) return end
    ...
    TriggerClientEvent("nc_delivery:GetOrder",src,Orders[id])
end)



RegisterNetEvent('nc_delivery:ImTurnoffui')
AddEventHandler('nc_delivery:ImTurnoffui', function()
  PlayersUsedUi[source] = nil
end)

RegisterNetEvent('nc_delivery:GetMeCoordForshow')
AddEventHandler('nc_delivery:GetMeCoordForshow', function(id)
  local src = source
  PlayersUsedUi[src] = nil
  TriggerClientEvent("nc_delivery:ShowCoords",src,Orders[id].coord)
end)


RegisterNetEvent('nc_delivery:CancelOrder')
AddEventHandler('nc_delivery:CancelOrder', function(id)
  local src = source
...
function updateUi()
  for k,v in pairs(PlayersUsedUi) do
    TriggerClientEvent("nc_delivery:OpenOrders",k,Orders,os.time())
  end
end
...
function singleNotify(src,type,message,time)   TriggerClientEvent('nc_delivery:SendNotify', src, type,message,time) end
