	...
	if serial then
		...
		if Player then
			...
			if JobType == 'police' or JobType == 'doj' then
				...
				local result = MySQL.query.await('SELECT * FROM mdt_weaponinfo WHERE serial = ?', { serial })
				if result ~= nil then
					exports.oxmysql:execute('UPDATE mdt_weaponinfo SET serial = ?, owner = ?, information = ?, weapClass = ?, weapModel = ?, image = ? WHERE serial = ?', {serial, owner, notes, weapClass, weapModel, imageurl, serial})
				else
					local result = false
