...
RegisterServerEvent('qb-crypto:server:ExchangeSuccess', function(LuckChance, type)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local ItemData = Player.Functions.GetItemByName("cryptostick")
    local type = nil
    local reward = cryptotypes[math.random(#cryptotypes)]
    ...
    if ItemData ~= nil then
        ...
        Player.Functions.RemoveItem("cryptostick", 1)
        exports['qb-phone']:AddCrypto(src, reward, 50)
        --Player.Functions.AddMoney('crypto', Amount)
        TriggerClientEvent('QBCore:Notify', src, "You have exchanged your Cryptostick for: "..reward..' X 50', "success", 3500)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["cryptostick"], "remove")
        TriggerClientEvent('qb-phone:client:AddTransaction', src, Player, {}, "There are "..reward.." X 50 credited!", "Credit")
    end
end)

RegisterServerEvent('qb-cyrpto:server:AddNewCrypto', function(type, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local name = Player.PlayerData.name
    exports['qb-phone']:AddCrypto(src, type, amount)
    print('The Player '..name..' requested '..amount..type..' and was paid')
end)
