	...
	if Config.MakeStores then
		for k, v in pairs(Config.Stores) do
			...
			Targets["ChairStore"..k] =
				...
				{ options = { { event = "nc-chairs:openShop", icon = "fas fa-chair", label = "Browse Store", store = v, ped = Peds[#Peds] }, },
					distance = 2.0
				})
		end
	end
...
--Chair Store Opening
RegisterNetEvent('nc-chairs:openShop', function(data)
	lookEnt(data.ped)
	if Config.JimShops then	TriggerServerEvent("nc-shops:ShopOpen", "shop", "Chairs", data.store)
...
RegisterNetEvent("nc-chairs:Use", function(item)
