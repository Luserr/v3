...
local function GpsToggle(toggle)
    ...
    onGPS = toggle
    TriggerServerEvent("qb-polgps:server:ToggleGPS", toggle)
end
...
RegisterNetEvent('qb-polgps:use', function()
    if isPolice() then
        toggleGps(not gpsMenu)
    end
end)

RegisterNetEvent('qb-polgps:onGpsDrop', function()
    if isPolice() then
        GpsToggle(false)
    end
end)
...
CreateThread(function()
    while true do
        ...
        if LocalPlayer.state.isLoggedIn and onGPS and isPolice() then
            QBCore.Functions.TriggerCallback('qb-polgps:server:GetItem', function(hasItem)
                if not hasItem then
                    if not isPolice() then return end
                    onGPS = false
                    GpsToggle(false)
...
RegisterNetEvent("qb-polgps:toggle", function(on)
	if isPolice() then
		ACTIVE = on
		if not ACTIVE then
...
RegisterNetEvent("qb-polgps:updateAll", function(data)
	if isPolice() then
		if ACTIVE then
			RemoveAnyExistingEmergencyBlips()
			RefreshBlips(data)
