...
RegisterServerEvent("qb-casino:server:GreenSell")
AddEventHandler("qb-casino:server:GreenSell", function()
    local src = source
    local price = Config.greenChipPrice
    local Player = QBCore.Functions.GetPlayer(src)
        ...
        for k, v in pairs(Player.PlayerData.items) do 
            if Player.PlayerData.items[k] ~= nil then 
                if ItemList[Player.PlayerData.items[k].name] ~= nil then 
                    ...
                    TriggerClientEvent('QBCore:Notify', src, "You sold "..Player.PlayerData.items[k].amount.." Green chips for $"..price)
                    TriggerClientEvent("doj:casinoChipMenu", src)
                end
            end
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "You dont have any green casino chips...", "error")
        TriggerClientEvent("doj:casinoChipMenu", src)
    end
end)
