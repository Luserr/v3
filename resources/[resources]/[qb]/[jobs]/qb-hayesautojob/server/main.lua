...
QBCore.Functions.CreateCallback('qb-vehicletuning:server:IsTunerAvailable', function(source, cb)
    local amount = 0
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
            if (Player.PlayerData.job.name == "hayesauto" and Player.PlayerData.job.onduty) then
                amount = amount + 1
            end
        end
    end
...
QBCore.Commands.Add("sethayesauto", "Give Someone The Hayes Auto job", {{
    name = "id",
    ...
    if IsAuthorized(Player.PlayerData.citizenid) then
        ...
        if TargetId ~= nil then
            ...
            if TargetData ~= nil then
                TargetData.Functions.SetJob("hayesauto")
...
QBCore.Commands.Add("firehayesauto", "Fire A Hayes Auto employee", {{
    name = "id",
    help = "ID Of The Player"
}}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    if IsAuthorized(Player.PlayerData.citizenid) then
        ...
        if TargetId ~= nil then
            local TargetData = QBCore.Functions.GetPlayer(TargetId)
            if TargetData ~= nil then
                if TargetData.PlayerData.job.name == "hayesauto" then
