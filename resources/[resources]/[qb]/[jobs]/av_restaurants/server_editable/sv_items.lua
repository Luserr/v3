...
CreateThread(function()
    if Config.Framework == 'QBCore' then
        QBCore.Functions.CreateCallback('av_restaurant:getItems', function(source, cb, data)
            local items = MySQL.query.await('SELECT * FROM av_items WHERE job = ? AND type = ?', {data['job'], data['type']})
            cb(items)
        end)
    elseif Config.Framework == 'ESX' then
        ESX.RegisterServerCallback('av_restaurant:getItems', function(source,cb,data)
            local items = MySQL.query.await('SELECT * FROM av_items WHERE job = ? AND type = ?', {data['job'], data['type']})
            cb(items)
        end)
    end
