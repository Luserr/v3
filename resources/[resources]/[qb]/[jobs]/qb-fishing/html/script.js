    ...
    if (type == "torunament") {
        if (data.active) {
            if (!TournActive) {
                ...
                $(".tourn").fadeIn(300);
                $("#tourn-tournname").html('Fishing Tournament');
                $("#tourn-firstplace").html('1st: ' + data.data.first);
                $("#tourn-secondplace").html('2nd: ' + data.data.second);
                $("#tourn-thirdplace").html('3rd: ' + data.data.third);
                $("#tourn-position").html('Pos: ' + data.data.Position + '/' + data.data.NumFishers);
                $("#tourn-points").html('Points: ' + data.data.MyPoints);
                $("#tourn-time").html('Time: ' + data.data.time);
            } else {
                $("#tourn-position").html('Pos: ' + data.data.Position + '/' + data.data.NumFishers);
                $("#tourn-tournname").html('Fishing Tournament');
                $("#tourn-firstplace").html('1st: ' + data.data.first);
...
function init($param) {
    ...
    $("#containerSecond > p").html("KLIK");
    if($param!=1)
        $("#containerSecond > p").append("<br><h4>at the right time</h4>");
    else
        $("#containerSecond > p").append("<br><h4>at the right time</h4>");
    $('.time').addClass('done');
}
...
$(document).ready(function() {
    ...
    $('#retry').click(function() {
        ...
        $("#containerSecond > p").removeClass("hidden");
...
Thermite.Open = function(data) {
    ...
    $("#containerSecond").css({"display":"block"});
    $("#retry2").addClass("hidden");   $("#circle").removeClass("hidden");
    $("#circle2").removeClass("hidden");
    $("#containerSecond > p").removeClass("hidden");
    $('.time').addClass('done');
...
Thermite.Failed = function() {
    if (active) $.post("https://qb-fishing/thermitefailed");
    active = false;
    Thermite.Close();
}

Thermite.Success = function() {
    if (active) $.post("https://qb-fishing/thermitesuccess");
    active = false;
    Thermite.Close();
}
...
Thermite.Close = function() {
    ...
    $.post('https://qb-fishing/closethermite');
}