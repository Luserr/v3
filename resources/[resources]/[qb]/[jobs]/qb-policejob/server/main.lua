...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= Config.LicenseRank then
        if args[2] == "driver" or args[2] == "weapon" or args[2] == "hunting" or args[2] == 'fishing' then
    ...
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) and OtherPlayer then
        if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
            local info = {
                ...
                bloodnumber = OtherPlayer.PlayerData.charinfo.blood
            }
...
RegisterNetEvent('police:server:BillPlayer', function(playerId, price)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer then
            ...
            exports['Renewed-Banking']:addAccountMoney("police", price, "Police Fines")
    ...
    if IsVehicleOwned(plate) then
        if not fullImpound then
            ...
            MySQL.Async.execute(
                ...
                {2, price, body, engine, fuel, garage, plate})
...
RegisterNetEvent('evidence:server:CreateBloodDrop', function(citizenid, bloodtype, coords, bloodnumber)
    local bloodId = CreateBloodId()
    BloodDrops[bloodId] = {
        dna = citizenid,
        bloodtype = bloodtype
    }
    TriggerClientEvent("evidence:client:AddBlooddrop", -1, bloodId, citizenid, bloodtype, coords, bloodnumber)
end)
