...
RegisterNetEvent('hospital:server:SendToBed', function(bedId, isRevive)
	...
	exports['Renewed-Banking']:addAccountMoney("ambulance", Config.BillCost, "Pillbox Bed Bill")
	TriggerClientEvent('hospital:client:SendBillEmail', src, Config.BillCost)
end)

RegisterNetEvent('hospital:server:JailSendToBed', function(bedId, isRevive)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('hospital:server:PutInBed', function(bedId, isRevive, info)
	...
	print(info.billprice)
	--TriggerEvent('qb-management:server:addAccountMoney', "ambulance", Config.BillCost)
	exports['Renewed-Banking']:addAccountMoney("ambulance", info.billprice, "Pillbox Bed Bill")
...
RegisterNetEvent('hospital:server:RespawnAtHospital', function()
	...
	for k, v in pairs(Config.Locations["beds"]) do
		if not v.taken then
			...
			Player.Functions.RemoveMoney("bank", Config.BillCost, "Pillbox")
			--TriggerEvent('qb-management:server:addAccountMoney', "ambulance", Config.BillCost)
			exports['Renewed-Banking']:addAccountMoney("ambulance", Config.BillCost, "Pillbox Medical")
			TriggerClientEvent('hospital:client:SendBillEmail', src, Config.BillCost)
			return
		end
	end
...
RegisterNetEvent('hospital:server:RespawnAtJail', function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	for k, v in pairs(Config.Locations["jailbeds"]) do
		if not v.taken then
			TriggerClientEvent('hospital:client:JailSendToBed', src, k, v, true)
			TriggerClientEvent('hospital:client:JailSetBed', -1, k, true)
			if Config.WipeInventoryOnRespawn then
				Player.Functions.ClearInventory()
				MySQL.Async.execute('UPDATE players SET inventory = ? WHERE citizenid = ?', { json.encode({}), Player.PlayerData.citizenid })
				TriggerClientEvent('QBCore:Notify', src, Lang:t('error.possessions_taken'), 'error')
	...
	--print("All beds were full, placing in first bed as fallback")
	TriggerClientEvent('hospital:client:JailSendToBed', src, 1, Config.Locations["jailbeds"][1], true)
	TriggerClientEvent('hospital:client:JailSetBed', -1, 1, true)
	if Config.WipeInventoryOnRespawn then
		Player.Functions.ClearInventory()
		MySQL.Async.execute('UPDATE players SET inventory = ? WHERE citizenid = ?', { json.encode({}), Player.PlayerData.citizenid })
		TriggerClientEvent('QBCore:Notify', src, Lang:t('error.possessions_taken'), 'error')
...
RegisterNetEvent('venture:server:RespawnAtJail', function(data)
	local src = source
