    ...
    if not isDead then
        ...
        if isDead then
            ...
            if IsPedInAnyVehicle(player, false) then
                ...
                TaskPlayAnim(player, "random@crash_rescue@car_death@std_car", "loop", 1.0, 1.0, -1, 1, 0, 0, 0, 0)
            else
...
RegisterCommand('debugcheck', function()
    debugchecking()
end)
...
function DeathTimer()
    hold = 5
    PlayerData = QBCore.Functions.GetPlayerData(data)
    if PlayerData.metadata['injail'] > 0 then
        injail = true
    else 
        injail = false
    ...
    while isDead do
        ...
        if deathTime <= 0 then
			...
			exports['Renewed-Banking']:addAccountMoney("ambulance", Config.BillCost, "Pillbox Bills")
