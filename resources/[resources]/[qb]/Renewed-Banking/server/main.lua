...
local function addAccountMoney(account, amount, reason)
    if not cachedAccounts[account] then
        Lang:t("logs.invalid_account",{account=account})
        return false
    end
    ...
    handleTransaction(account, account, amount, reason, account, account, "deposit")
...
local function removeAccountMoney(account, amount, reason)
    if not cachedAccounts[account] then
        print(Lang:t("logs.invalid_account",{account=account}))
        return false
    end
    ...
    cachedAccounts[account].amount -= amount
    updateBalance(account)
    handleTransaction(account, account, amount, reason, account, account, "withdraw")
    return true
