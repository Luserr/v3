...
Padlock.Close = function() {
    ...
    $.post('https://qb-storagelockers/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('https://qb-storagelockers/PadLockClose');
    if (data.error != null) {
        $.post('https://qb-storagelockers/CombinationFail');
    }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("https://qb-storagelockers/UseCombination", JSON.stringify({
        type: type,
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('https://qb-storagelockers/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('https://qb-storagelockers/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
...
function outOfPins() {
    gameOver = true;
    $.post('https://qb-storagelockers/fail');
    setTimeout(function(){
        reset()
    }, 250)
}
...
function unlock() {
    gameOver = true;
    $.post('https://qb-storagelockers/success');
    solveDeg = (Math.random() * 180) - 90
    solvePadding = 4
    maxDistFromSolve = 45
    pinRot = 1
