...
function lockpickFinish(success)
    ...
    if usingAdvanced then
        if chance <= Config.RemoveLockpickAdvanced then
            ...
            TriggerServerEvent("qb-smallresources:server:RemoveItem", "advancedlockpick", 1)
            --QBCore.Functions.RemoveItem('advancedlockpick')
        end
    else
        if chance <= Config.RemoveLockpickNormal then
            TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["lockpick"], "remove")
            TriggerServerEvent("qb-smallresources:server:RemoveItem", "lockpick", 1)
            --QBCore.Functions.RemoveItem('lockpick')
        end
    end
end
