...
RegisterNetEvent('qb-phone:server:SendEmploymentPayment', function(Job, CID, amount)
    ...
    if Config.RenewedBanking then
        if not exports['Renewed-Banking']:removeAccountMoney(Job, amt, "Phone Payment") then return notifyPlayer(src, "Insufficient Funds...") end
    ...
    if paid then
        if amount == bills[source].amount then
            ...
            if Config.RenewedBanking then
                ...
                exports['Renewed-Banking']:addAccountMoney(bills[source].job, amount, "Invoice")
            else
                exports['Renewed-Banking']:addAccountMoney(bills[source].job, amount, "Invoice")
            end
