...
RegisterNetEvent('qb-radialmenu:client:TakeStretcher', function()
    ...
    if vehicle ~= 0 then
        ...
        if obj ~= 0 then
            ...
            QBCore.Functions.Notify("Couldn't create the requested object", 'error')
        end
    else
        ...
        if stretcherObject then
            ...
            if distance <= 1.0 then
                if not isAttached then
                    sleep = 0
                    DrawText3Ds(offsetCoords.x, offsetCoords.y, offsetCoords.z, "[E] Push Stretcher")
                    ...
                    sleep = 0
                    DrawText3Ds(offsetCoords.x, offsetCoords.y, offsetCoords.z, "[E] Stop Pushing")
                    if IsControlJustPressed(0, 51) then
                        detachStretcher()
                        isAttached = false
                        sleep = 100
                ...
                if not isLayingOnBed then
                    if not isAttached then
                        sleep = 0
                        DrawText3Ds(offsetCoords.x, offsetCoords.y, offsetCoords.z + 0.2, "[G] Lay On Stretcher")
            ...
            elseif distance <= 2 then
                if not isLayingOnBed then
                    ...
                    if not isAttached then
                        sleep = 0
                        DrawText3Ds(offsetCoords.x, offsetCoords.y, offsetCoords.z + 0.2, "[G] Get Off Stretcher")
                        if IsControlJustPressed(0, 47) or IsDisabledControlJustPressed(0, 47) then
                            getOffStretcher()
                            sleep = 100
                        end
