...
if GetConvar('voice_useNativeAudio', 'false') == 'true' then
	...
	Cfg.voiceModes = {
		...
		{4.5, "Normal"}, -- Normal speech distance in gta distance units
		{8.0, "Shouting"} -- Shout speech distance in gta distance units
	}
else
	Cfg.voiceModes = {
		{3.0, "Whisper"}, -- Whisper speech distance in gta distance units
		{8.5, "Normal"}, -- Normal speech distance in gta distance units
		{17.0, "Shouting"} -- Shout speech distance in gta distance units
	}
end
