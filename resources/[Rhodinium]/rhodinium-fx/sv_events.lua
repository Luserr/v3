...
RegisterServerEvent("qpixel-fx:setSteamId")
AddEventHandler("qpixel-fx:setSteamId", function()
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local steamId = user:getVar("steamid")
    TriggerClientEvent("qpixel-fix:setSteamIdClient", src, steamId)
end)

RegisterServerEvent("qpixel-fx:smoke:grenade")
AddEventHandler("qpixel-fx:smoke:grenade", function(pCoords)
...
RegisterNetEvent("qpixel-fx:chain:blingDiamonds", function(pCoords, pType, pSize, pStrength, pScale)
