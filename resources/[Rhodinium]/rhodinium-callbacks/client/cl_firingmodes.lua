...
Citizen.CreateThread(function()
	-- Register Keybinds
    exports["rhodinium-callbacks"]:registerKeyMapping("", "Weapons", "Switch Weapon Mode", "+switchWeaponMode", "-switchWeaponMode")
...
function SwitchWeaponMode()
	...
	print(json.encode(pWeaponHash))

	if AllowedModes[pWeaponHash] == nil then
		SELECTED_FIRE_RATE = 3
        QBCore.Functions.Notify("You have no modes to cycle through", 'error')
		return
	end
	...
	if ForceWeaponMode[pWeaponHash] ~= nil then
		if SELECTED_FIRE_RATE == ForceWeaponMode[pWeaponHash] then
			QBCore.Functions.Notify("You can't change mode from: " .. FireRateTypes[SELECTED_FIRE_RATE].label , 'error')
		else
			SELECTED_FIRE_RATE = ForceWeaponMode[pWeaponHash]
			TriggerEvent("rhodinium-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
		end
		return
	end
	...
	QBCore.Functions.Notify("Changed fire mode: " .. selectedRate.label , 'success')
	TriggerEvent("rhodinium-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)

	if selectedRate.label == "Single Fire" then
		TriggerEvent('rhodinium-weapons:client:setWeaponFireRate', 30)
	elseif selectedRate.label == "Burst Fire" then
		TriggerEvent('rhodinium-weapons:client:setWeaponFireRate', 70)
	elseif selectedRate.label == "Full Auto" then
		TriggerEvent('rhodinium-weapons:client:setWeaponFireRate', 100)
	end	
end
...
RegisterNetEvent('rhodinium-items:SetAmmo')
AddEventHandler('rhodinium-items:SetAmmo', function(sentammoTable)
...
RegisterNetEvent("rhodinium-weapons:client:weaponEquiped")
AddEventHandler("rhodinium-weapons:client:weaponEquiped", function(pWeaponHash, newInformation, sqlID, itemToRemove, pArmed)
	

	if pArmed2 == true then
		pArmed2 = false 
		print("Not holding a weapon")
		return
	else
		pArmed2 = true
		print("Holding a weapon")
		if ForceWeaponMode[pWeaponHash] ~= nil then
			SELECTED_FIRE_RATE = ForceWeaponMode[pWeaponHash]
			TriggerEvent("rhodinium-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
		end
		...
		TriggerEvent("rhodinium-weapons:client:showWeaponFireRate", pArmed)
		TriggerEvent("rhodinium-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
	end
