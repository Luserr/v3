...
RPC.register("qpixel-business:completePurchase", function(pSource, pData)
    ...
    local insert = {registerId = data.registerId}
    ...
    if data.cash == true then
        ...
        if cash < tonumber(amount) then 
            print(cash)
            print(amount)
            return false, "You can't afford this purchase" 
        end
        TriggerClientEvent("qpixel-business:closePurchase", -1, insert)
        xPlayer.Functions.RemoveMoney('cash', amount, 'uwucafe-payment')
    else
        if bank < tonumber(amount) then 
            return false, "You can't afford this purchase" 
        end
        TriggerClientEvent("qpixel-business:closePurchase", -1, insert)
        xPlayer.Functions.RemoveMoney('bank', amount, 'uwucafe-payment')
    end
