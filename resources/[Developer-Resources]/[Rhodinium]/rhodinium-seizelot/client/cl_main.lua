...
RegisterNetEvent('venture-seizelot:client:GetSeizedList', function()
    ...
    for i=1, #vehcheck do
        if vehcheck[i].state == 2 then
            table.insert(menu, {
                ...
                params = {
                    event = "venture-seizelot:client:PoliceReturn",
                    args = {
...
RegisterNetEvent('venture-seizelot:client:PoliceReturn', function(data)
    ...
    while true do
        ...
        if distance <= 5.0 then
            TriggerEvent('venture-seizelot:client:ReturnVehicle', data)
            return false
        end
    end
end)
...
RegisterNetEvent('venture-seizelot:client:ReturnVehicle', function(data)
    local PlayerData = QBCore.Functions.GetPlayerData()
    ...
    if seizeinfo == nil then
        ...
        if seizeinfo.decision == 'Yes' then
            TriggerServerEvent('venture:server:CreateInvoice', billedplayer, PlayerData.job.name, PlayerData.charinfo.firstname, PlayerData.citizenid, depotprice)
            TriggerServerEvent('venture-seizelot:server:ReturnVehicle', data)
