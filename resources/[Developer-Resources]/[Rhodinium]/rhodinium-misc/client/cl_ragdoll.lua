...
AddEventHandler("qpixel-menu:var:inServerFarm", function(pBool)
...
AddEventHandler('trycpr', function()
    ...
    if curDist < 5 or curDist2 < 5 then
        ...
        while penis < 10 do
            penis = penis + 1
            local finished = exports["qpixel-ui"]:taskBarSkill(math.random(2000, 6000), math.random(5, 10))
...
function releaseBody(dropItems)
    ...
    if dropItems then
        ...
        --TriggerServerEvent("qpixel-ragdoll:removeallitems")
    end
    ...
    local doctors = RPC.execute('qpixel-jobmanager:jobCount', 'doctor')
    ...
    TriggerServerEvent('qpixel-healthcare:playerRespawn', hosp)
...
local devmode = false
RegisterNetEvent("qpixel:admin:currentDevmode")
AddEventHandler("qpixel:admin:currentDevmode", function(pDevmode)
    devmode = pDevmode
end)
...
AddEventHandler('ragdoll:sendPing', function()
    if not SentPing then
        SentPing = true
        exports["qpixel-dispatch"]:policedead('10-47', "Injured Civillian", "429")
        Citizen.Wait(10000)
        SentPing = false
    else
