...
function LoadDTModule()
    ...
    exports["rhodinium-callbacks"]:registerKeyMapping("", "DriveThru Headset", "Push-To-Talk", "+transmitToDriveThru", "-transmitToDriveThru")
    ...
    exports["rhodinium-callbacks"]:registerKeyMapping("", "DriveThru Headset", "On / Off", "+toggleDTRadioState", "-toggleDTRadioState")
    ...
    exports["rhodinium-callbacks"]:registerKeyMapping("", "DriveThru Headset", "Volume Up", "+increaseDTVolume", "-increaseDTVolume")
    RegisterCommand("+increaseDTVolume", IncreaseDTRadioVolume, false)
    RegisterCommand("-increaseDTVolume", function() end, false)

    exports["rhodinium-callbacks"]:registerKeyMapping("", "DriveThru Headset", "Volume Down", "+decreaseDTRadioVolume", "-decreaseDTRadioVolume")
    ...
    RegisterNetEvent("rhodinium-voice:drivethru:connect", ConnectToDTFrequency)

    RegisterNetEvent("rhodinium-voice:drivethru:disconnect", DisconnectFromDTFrequency)

    RegisterNetEvent("rhodinium-voice:drivethru:setPowerState", SetDTRadioPowerState)

    RegisterNetEvent("rhodinium-voice:drivethru:addSubscriber", AddDTSubscriber)

    RegisterNetEvent("rhodinium-voice:drivethru:removeSubscriber", RemoveDTSubscriber)

    -- scuffed way of overriding vanilla keybind
    AddEventHandler("rhodinium:voice:transmissionStarted", function()
        if DTIsHeadset then return end
        StartDTTransmission()
    end)

    AddEventHandler("rhodinium:voice:transmissionFinished", function()
        if DTIsHeadset then return end
        StopDTTransmission(true)
    end)
